...
__doc__ = """Скрипт перемещает N случайных шрифтов в скрытый ключ реестра.
Каждый следующий запуск возвращает его, но перемещает туда N других шрифтов, генерируя новый отпечаток твоей системы"""


class FontFingerprintGenerator:
    """
    Класс изменяет отпечаток системного шрифта, перемещая несколько шрифтов в скрытый подраздел ключа Fonts.
    Если некоторые шрифты были в скрытом ключе, они возвращаются, поэтому все шрифты остаются в системе
    """
    ...
    def recover_fonts():
        """
        Восстановление скрытых шрифтов перед распространением для создания новых отпечатков шрифтов
        """
    ...
    def generate_font_fingerprint(fonts_delete):
        """
        Скрипт перемещает N случайных шрифтов в скрытый ключ реестра.
        : param fonts_delete: Шрифты для удаления
        """
